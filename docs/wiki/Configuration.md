...
| `GITHUB_TOKEN` | — | Optional. Raises the GitHub API rate limit (60 requests/hour per IP unauthenticated → 5000/hour) used by the update check and the branch switcher's branch list. Only needed if you hit the limit; a read-only token with no scopes is enough for a public repo |
