...
- **Tunnel Cloudflare**, come alternativa a ngrok per esporre il server di sviluppo in modo sicuro — utile quando la rete o l'antivirus blocca specificamente ngrok (vedi il fix qui sotto per il caso reale che ha portato a questa funzione). Nuova scheda **Account → Tunnel Cloudflare** (avvia/ferma il proprio, con un token opzionale per un tunnel con nome — senza, parte un Quick Tunnel anonimo, nessun account richiesto, con un link diretto alla dashboard Cloudflare per crearne uno); la supervisione admin vive nella stessa scheda **Impostazioni → Tutti i tunnel** di ngrok, come sottoscheda separata. Il binario `cloudflared` non richiede installazione manuale: viene scaricato in automatico al primo avvio (pacchetto `cloudflared` su npm, stesso principio con cui `@ngrok/ngrok` include già il proprio binario)
- **I codici pubblici possono ora essere illimitati** — bottone ♾️ nel menu di creazione del bot Telegram, o scrivendo `0` come numero personalizzato — invece di un tetto sempre obbligatorio. Il limite è applicato dove il codice viene davvero verificato (creazione della sessione partecipante), quindi vale a prescindere da dove il codice pubblico è stato creato
- **Il bot Telegram genera messaggi pronti da inoltrare ai partecipanti** («📤 Crea messaggio inoltrabile», dopo aver creato dei codici): un messaggio alla volta, codice e link inclusi, con «➡️ Messaggio successivo» per scorrere i successivi — nessun bottone del bot finisce nel testo, perché Telegram inoltra solo il testo di un messaggio, mai la sua tastiera. Il testo è **personalizzabile per evento** («✏️ Personalizza messaggio» nel bot, o il nuovo campo alla modifica evento nel pannello web, sempre sincronizzati), con segnaposto `{link}`, `{event}`, `{code}`; senza personalizzazione usa un messaggio predefinito
- I messaggi del bot Telegram ora usano la formattazione HTML nativa: i codici sono monospazio e si copiano con un tap, i link hanno un bottone «Apri link» dedicato, e i comandi (`/start`, `/menu`, `/help`, `/logout`) compaiono nel menu «/» di Telegram — registrati automaticamente quando registri il webhook da Impostazioni → Telegram, invece di doverli ricordare a memoria

### Security
- **Nessun form del pannello aveva `method="post"`** (tranne l'inserimento codice del partecipante) — si affidavano interamente a JavaScript per intercettare l'invio e mandarlo via `fetch`. Se l'hydration di React non fa in tempo ad agganciare l'handler (osservato dal vivo su un tunnel con HMR instabile: decine di ricariche in pochi minuti), il browser esegue l'invio nativo di default, che è **GET** — ogni campo del form, password comprese, finisce nella query string dell'URL, quindi nella cronologia del browser e potenzialmente nei log di chi instrada il traffico (un tunnel, un proxy). Riprodotto e confermato sul login. Tutti i 16 form del pannello (login, reset password, creazione utenti, tutte le impostazioni con token/secret, ecc.) ora hanno `method="post"` esplicito: se JavaScript non parte in tempo, l'invio nativo va comunque nel corpo della richiesta, mai nell'URL. **Se hai inserito una password vera in un URL con questo bug (visibile con `?password=...` nella barra degli indirizzi), cambiala** — potrebbe essere salvata nella cronologia del browser o nei log di un eventuale tunnel/proxy nel mezzo
...
- **Il Tunnel Cloudflare avviato dal pannello restava bianco/in caricamento infinito una volta aperto l'URL pubblico**, sia con token che come Quick Tunnel anonimo. In sviluppo Next.js blocca le richieste cross-origin verso le risorse `/_next/*` (i bundle JS, gli aggiornamenti RSC) se l'host non è nella whitelist `allowedDevOrigins` — l'HTML iniziale passava attraverso il tunnel, ma ogni chunk JS veniva rifiutato, quindi React non idratava mai. Aggiunto `*.trycloudflare.com` come pattern d'eccezione (Next.js supporta i wildcard su `allowedDevOrigins`), che copre ogni hostname Quick Tunnel senza doverlo conoscere in anticipo; un tunnel con hostname fisso va ancora aggiunto a mano
- **Eliminare un utente (o qualsiasi altra azione "distruttiva": revocare o eliminare un codice pubblico, eliminare un evento, un partecipante o un annuncio, azioni sessione, aggiornamento app) poteva sembrare non fare nulla.** Il pulsante di conferma usava `window.confirm()`, il dialogo nativo del browser — inaffidabile in alcuni browser/ambienti (viene chiuso automaticamente o bloccato senza alcun errore visibile). Sostituita ovunque con una conferma a due click integrata nel pulsante stesso: il primo click lo "arma" (colore pieno, ben visibile, con il messaggio di conferma), il secondo entro qualche secondo conferma davvero
- **Le tabelle del pannello admin (eventi, partecipanti, codici pubblici, utenti) non erano utilizzabili da telefono**: colonne e pulsanti azione non entravano nella larghezza dello schermo, e raggiungere un'azione richiedeva scorrere orizzontalmente fino a perdere di vista a quale riga apparteneva. Sotto un certo punto di rottura ogni riga diventa ora una scheda verticale; da tablet in su la tabella resta quella di sempre
